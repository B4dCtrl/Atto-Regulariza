...
import {
  ...
  User, Building2, Loader2, AlertCircle,
} from "lucide-react";
...
const SITUACOES = [
  ...
  { id: "sem_habite",    label: "Sem habite-se / averbação",  desc: "Construção ou reforma sem legalização" },
  { id: "retificacao",   label: "Retificação de área",        desc: "Área real diferente do que consta na matrícula" },
  { id: "heranca",       label: "Herança / Inventário",       desc: "Imóvel de familiar falecido sem partilha concluída" },
          ...
          <div>
            ...
            {!listLoading && (
              <p className="mt-1 text-sm text-ink-soft">
                {list.length} processo{list.length !== 1 ? "s" : ""} registrado{list.length !== 1 ? "s" : ""}
              </p>
            )}
          </div>
          <button
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-ink-soft">
                      {p.client_email ?? "—"}{p.city ? ` · ${p.city}` : ""}{p.state ? `/${p.state}` : ""}
                    </div>
                  </div>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="cs2" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.2 }} className="space-y-6">
              ...
              <Field label="Observações adicionais">
                <textarea value={form.observacoes} onChange={(e) => set("observacoes", e.target.value)} rows={3}
                  placeholder="Informações extras que o profissional deve saber…" className={`${inp()} resize-none`} />
              </Field>
            </motion.div>
          )}
          ...
          {step === 3 && (
            <motion.div key="cs3" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.2 }} className="space-y-5">
              ...
              <AnimatePresence mode="wait">
                {!tutorialDone ? (
                  <motion.div key={tutorialStep} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                    {(() => {
              ...
              {!tutorialDone && (
                <div className="flex items-center justify-between">
                  <button onClick={() => setTutorialStep((s) => Math.max(s - 1, 0))} disabled={tutorialStep === 0}
